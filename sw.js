const CACHE_NAME = 'geofaceattend-v4';
...
self.addEventListener('install', event => {
    console.log('🛠️ Service Worker v4 installing...');
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then(cache => {
                console.log('📦 Caching critical assets...');
                // Using addAll with a map to skip failures on individual files
...
self.addEventListener('activate', event => {
    ...
    event.waitUntil(
        caches.keys().then(cacheNames => {
            ...
            console.log('✅ Service Worker v4 ready');
...
self.addEventListener('fetch', (event) => {
    ...
    // 1. Skip API requests (don't cache them)
    if (url.port === '5001' || url.pathname.includes('/api/')) {
        event.respondWith(fetch(event.request));
        return;
    }

    // 2. Local/File origin handling
    if (event.request.url.startsWith('file://')) {
        return; // Let the browser handle file:// requests
    }
