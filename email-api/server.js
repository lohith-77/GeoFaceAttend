...
app.use(cors({
    origin: [
        'https://geofaceattend-1.onrender.com',
        'http://localhost:5010',
        'http://localhost:3000'
    ],
    credentials: true,
...
// Disable CSP completely for local development to allow CDNs and inline scripts
app.use(helmet({
...
app.post('/register', async (req, res) => {
    try {
        ...
        if (isPostgreSQL) {
            ...
            result = await query('SELECT id, emp_id, name, email, department, role FROM users WHERE id = ?', [insertResult.insertId]);
        }
...
app.post('/api/attendance/checkin', authenticateToken, async (req, res) => {
    try {
        ...
        if (isPostgreSQL) {
            ...
            result = await query('SELECT * FROM attendance WHERE id = ?', [insertResult.insertId]);
        }
...
app.post('/api/attendance/checkout', authenticateToken, async (req, res) => {
    try {
        ...
        if (isPostgreSQL) {
            ...
            result = await query('SELECT * FROM attendance WHERE id = ?', [active.rows[0].id]);
        }
...
app.post('/api/leaves/apply', authenticateToken, async (req, res) => {
    try {
        ...
        if (isPostgreSQL) {
            ...
            result = await query('SELECT * FROM leave_requests WHERE id = ?', [insertResult.insertId]);
        }
...
app.post('/api/qr/generate', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        if (isPostgreSQL) {
            ...
            result = await query('SELECT * FROM qr_codes WHERE id = ?', [insertResult.insertId]);
        }
